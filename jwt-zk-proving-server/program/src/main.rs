...
fn extract_jwt_claims(jwt_payload: &[u8]) -> Result<(String, String, String, String), &'static str> {
    let payload_str = core::str::from_utf8(jwt_payload).map_err(|_| "Invalid UTF-8")?;
    let payload_bytes = base64::engine::general_purpose::URL_SAFE_NO_PAD
        .decode(payload_str)
        .map_err(|_| "Base64 decode failed")?;
    let decoded_str = core::str::from_utf8(&payload_bytes).map_err(|_| "Invalid UTF-8 in payload")?;
